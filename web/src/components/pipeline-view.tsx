...
export function PipelineView({
  ...
  const tab: Tab = (TABS as readonly string[]).includes(pTab)
    ? (pTab as Tab)
    : "INBOX";
  ...
  const sortKey: SortKey = (SORT_KEYS as readonly string[]).includes(pSort)
    ? (pSort as SortKey)
    : "score";
  const sort = {
    key: sortKey,
    dir: (params.get("dir") === "1" ? 1 : -1) as 1 | -1,
  };
        ...
        {TABS.map((t) => {
          const count =
            t === "INBOX"
              ...
              : t === "ALL"
                ? applications.length
                : applications.filter((r) => canonStatus(r.status).includes(t))
                    .length;
          return (
            <button
              key={t}
      ...
      {tab === "INBOX" ? (
        ...
        pendingInbox.length > 0 ? (
          <InboxTriage
            inbox={pendingInbox}
            initialWithinDays={
              Number.isFinite(parseFloat(params.get("within") ?? ""))
                ? parseFloat(params.get("within")!)
                : null
            }
          />
        ) : (
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((r, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-faint tabular-nums">
                    {r.date}
                  </td>
                </tr>
              ))}
      ...
      <div className="px-6 py-10 text-center">
        ...
        {count > 0 ? (
          ...
          <>
            ...
            >
              <Compass className="size-4" /> Run your first free scan{" "}
              <ArrowRight className="size-4" />
            </Link>
            <p className="mx-auto mt-4 max-w-sm text-xs text-muted">
              Prefer the terminal? Run{" "}
              <code className="rounded bg-surface-hover px-1 py-0.5 font-mono">
                career-ops scan
              </code>
              , or add job URLs to{" "}
              <code className="rounded bg-surface-hover px-1 py-0.5 font-mono">
